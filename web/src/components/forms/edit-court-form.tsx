...
export default function EditCourtForm({ open, onOpenChange, onSubmit, court }: EditCourtFormProps) {
  ...
  useEffect(() => {
    if (court) {
      ...
      setValue('type', court.type as any)
      setValue('surface', court.surface as any)
      setValue('status', court.status as any)
      setValue('description', court.description || '')
      setValue('hourlyRate', court.hourlyRate)
      setValue('dimensions', court.dimensions)
      setValue('lighting', court.lighting)
      setValue('airConditioning', court.airConditioning)
      setValue('accessibility', court.accessibility)
      setValue('notes', court.notes || '')
      setValue('lastMaintenance', court.lastMaintenance || '')
      setValue('nextMaintenance', court.nextMaintenance || '')
  ...
  const handleFormSubmit = async (data: CourtFormData) => {
    ...
    try {
      ...
      setError(err.message || 'An error occurred while updating the court')
    } finally {
      setIsLoading(false)
    }
  }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Update {court.name}'s information and settings.
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-4">
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={isLoading}>
              {isLoading ? 'Updating Court...' : 'Update Court'}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
